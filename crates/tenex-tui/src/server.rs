...
fn resolve_project_coordinate(state: &ServerState, project_dtag: &str) -> Result<String> {
...
fn get_pm_agent_pubkey(state: &ServerState, project_a_tag: &str) -> Result<String> {
    let store = state.data_store.lock().unwrap();
