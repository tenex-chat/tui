...
impl MergedAgentSettingsInputs {
    ...
    /// Build the `AgentSettingsState` for `agent` from these merged inputs.
    pub fn into_state(
        self,
        ...
        crate::ui::modal::AgentSettingsState::new(
            ...
            self.all_available_skills,
            self.all_available_mcp_servers,
        )
    }
}
...
impl App {
    ...
    pub fn open_selected_project_agent_picker(&mut self) -> bool {
        ...
        let mut dialog = crate::ui::modal::ProjectDialogState::new_editing(
            project_a_tag,
            project.title.clone(),
            project.description.clone().unwrap_or_default(),
            project.repo_url.clone(),
            is_private,
            project.agent_pubkeys.clone(),
            project.mcp_tool_ids.clone(),
        );
        dialog.tab = crate::ui::modal::ProjectDialogTab::Agents;
        self.modal_state = ModalState::ProjectDialog(dialog);
        true
    }
...
#[cfg(test)]
mod merged_agent_settings_tests {
    use super::*;
    use serde_json::json;
    use tenex_core::models::AgentConfig;

    fn make_agent() -> ProjectAgent {
        ProjectAgent {
            ...
            mcp_servers: vec!["legacy-mcp".to_string()],
        }
    }
    ...
    fn make_agent_config(
        ...
        for s in skills {
            ...
            if active_skills.contains(s) {
                t.push("active".to_string());
            }
            tags.push(t);
        }
        for m in mcps {
            let mut t = vec!["mcp".to_string(), (*m).to_string()];
            if active_mcps.contains(m) {
                t.push("active".to_string());
            }
            tags.push(t);
        }
    ...
    fn derives_agent_settings_from_kind0_and_inventory_models() {
        ...
        assert_eq!(state.available_mcp_servers, vec!["mcp1".to_string()]);
