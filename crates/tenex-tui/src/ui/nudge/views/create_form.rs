...
fn render_basics_step(f: &mut Frame, area: Rect, state: &NudgeFormState) {
    ...
    if hashtags_focused {
        let cursor_x = area.x
            + 2
            + state.hashtags.iter().map(|t| t.len() + 4).sum::<usize>() as u16
...
fn render_content_step(f: &mut Frame, area: Rect, state: &NudgeFormState) {
    let label = Paragraph::new("Behavioral Instructions").style(
        Style::default()
            .fg(theme::ACCENT_PRIMARY)
            .add_modifier(Modifier::BOLD),
    );
...
fn render_permissions_step(f: &mut Frame, app: &App, area: Rect, state: &NudgeFormState) {
    ...
    let label = Paragraph::new("Tool Permissions (Optional)").style(
        Style::default()
            .fg(theme::ACCENT_PRIMARY)
            .add_modifier(Modifier::BOLD),
    );
    ...
    if state.permissions.is_additive_mode() && !is_adding {
        ...
        if !conflicts.is_empty() {
            ...
            let warning_para =
                Paragraph::new(warning).style(Style::default().fg(theme::ACCENT_WARNING));
            f.render_widget(
                warning_para,
                Rect::new(
                    area.x,
    ...
    if !available_tools.is_empty() && !is_adding {
        let tool_count = format!(
            "{} tools available from project statuses",
            available_tools.len()
        );
...
fn render_mode_selector(f: &mut Frame, area: Rect, state: &NudgeFormState) {
    ...
    let description =
        Paragraph::new(current_mode.description()).style(Style::default().fg(theme::TEXT_DIM));
...
fn render_additive_mode_content(f: &mut Frame, area: Rect, state: &NudgeFormState) {
    ...
    // Deny column
    let deny_area = Rect::new(area.x + col_width + 2, area.y, col_width, area.height);
    render_permission_column(
        f,
        deny_area,
        "✗ Deny Tools",
    ...
    if state.selecting_configured {
        ...
        let hint = format!(
            "{} tools · ↑↓ select · x remove · Esc back",
            configured_count
        );
...
fn render_exclusive_mode_content(f: &mut Frame, area: Rect, state: &NudgeFormState) {
    ...
    let hint_text = if state.selecting_configured {
        format!(
            "{} tools · ↑↓ select · x remove · Esc back",
            state.permissions.only_tools.len()
        )
    } else {
...
fn render_tool_selector(
    ...
    // Title
    let title = format!(
        "Select tool to {} (Esc to cancel)",
        mode_label.to_lowercase()
    );
    let title_para = Paragraph::new(title).style(
        Style::default()
            .fg(accent_color)
            .add_modifier(Modifier::BOLD),
    );
...
fn render_permission_column(
    ...
    if tools.is_empty() && !is_adding {
        let empty = Paragraph::new("(none)").style(Style::default().fg(theme::TEXT_DIM));
...
fn render_review_step(f: &mut Frame, area: Rect, state: &NudgeFormState) {
    let label = Paragraph::new("Review Your Nudge").style(
        Style::default()
            .fg(theme::ACCENT_PRIMARY)
            .add_modifier(Modifier::BOLD),
    );
    ...
    if state.permissions.is_exclusive_mode() {
        ...
        if only_count > 0 {
            ...
            // Show the tools
            let tools_preview: String = state
                .permissions
                .only_tools
                .iter()
                .take(5)
                ...
                .collect::<Vec<_>>()
                .join(", ");
            let suffix = if only_count > 5 {
                format!(" +{} more", only_count - 5)
            } else {
                String::new()
            };
        ...
        if allow_count > 0 || deny_count > 0 {
            let perms_line = Line::from(vec![
                ...
                Span::styled(" - ", Style::default().fg(theme::TEXT_MUTED)),
                Span::styled(
                    format!("{} allowed", allow_count),
                    Style::default().fg(theme::ACCENT_SUCCESS),
                ),
                Span::styled(", ", Style::default().fg(theme::TEXT_MUTED)),
                Span::styled(
                    format!("{} denied", deny_count),
                    Style::default().fg(theme::ACCENT_ERROR),
                ),
            ]);
            f.render_widget(
    ...
    // Content preview
    let content_label = Paragraph::new("Content:").style(Style::default().fg(theme::TEXT_MUTED));
    ...
    let content_area = Rect::new(
        ...
        y,
        area.width,
        area.height.saturating_sub(y - area.y + 2),
    );
...
fn render_step_hints(f: &mut Frame, area: Rect, state: &NudgeFormState) {
    let hints = match state.step {
        NudgeFormStep::Basics => "Tab cycle fields · Enter next step · Esc cancel",
