...
/// Live feed of most recent events (circular buffer)
#[derive(Debug, Clone)]
...
impl EventFeed {
    ...
    pub fn push(&mut self, event_id: String, kind: u16) {
    ...
    /// Get total count of events currently in buffer
...
/// Thread-safe wrapper for event feed
#[derive(Debug, Clone)]
pub struct SharedEventFeed {
    inner: Arc<RwLock<EventFeed>>,
}

impl Default for SharedEventFeed {
    fn default() -> Self {
        Self::new(1000) // Keep last 1000 events
    }
}

impl SharedEventFeed {
    pub fn new(max_size: usize) -> Self {
        Self {
            inner: Arc::new(RwLock::new(EventFeed::new(max_size))),
        }
    }

    pub fn push(&self, event_id: String, kind: u16) {
        self.inner.write().push(event_id, kind);
    }

    pub fn recent(&self) -> Vec<LiveEvent> {
        self.inner.read().recent()
    }
