...
pub fn aggregate_todo_state(messages: &[Message]) -> TodoState {
    ...
    for msg in messages {
        ...
        let tool_calls = match parsed {
            MessageContent::Mixed { tool_calls, .. } => tool_calls,
            MessageContent::PlainText(_) => continue,
        };
        ...
        for tool_call in tool_calls {
            ...
            match tool_name.as_str() {
                "todowrite" => {
                    ...
                    if let Some(todos) = tool_call.parameters.get("todos") {
                ...
                "todo_add" => {
                    if let Some(items_val) = tool_call.parameters.get("items") {
