...
static int ndb_init_lmdb(const char *filename, struct ndb_lmdb *lmdb, size_t mapsize)
{
	...
	/* Use MDB_NOLOCK to bypass POSIX named-semaphore locking.
	 * On macOS the semaphore table can fill up between reboots (ENOSPC),
	 * making ordinary open impossible.  The caller is responsible for
	 * ensuring only one writer accesses the database at a time (e.g. via
	 * an OS-level flock on a separate lock file). */
	if ((rc = mdb_env_open(lmdb->env, filename, MDB_NOLOCK, 0664))) {
